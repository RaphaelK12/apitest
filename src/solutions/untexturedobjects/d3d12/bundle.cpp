...
UntexturedObjectsD3D12Bundle::UntexturedObjectsD3D12Bundle()
{
}

bool UntexturedObjectsD3D12Bundle::Init(const std::vector<UntexturedObjectsProblem::Vertex>& _vertices,
	...
	size_t _objectCount)
{
	...
	if (!CreateCommandList(_objectCount))
		return false;

	return true;
}

bool UntexturedObjectsD3D12Bundle::CreatePSO()
{
...
bool UntexturedObjectsD3D12Bundle::CreateGeometryBuffer(const std::vector<UntexturedObjectsProblem::Vertex>& _vertices,
																const std::vector<UntexturedObjectsProblem::Index>& _indices)
{
...
bool UntexturedObjectsD3D12Bundle::CreateConstantBuffer(size_t count)
{
...
void UntexturedObjectsD3D12Bundle::Render(const std::vector<Matrix>& _transforms)
{
...
void UntexturedObjectsD3D12Bundle::Shutdown()
{
...
bool UntexturedObjectsD3D12Bundle::CreateCommandList( int count )
{
	for (int k = 0; k < NUM_ACCUMULATED_FRAMES; k++)
	{
		...
		// set root signature
		m_CommandBundle[k]->SetGraphicsRootSignature(m_RootSignature);

		unsigned int counter = 0;
		for (unsigned int u = 0; u < count; ++u) {
			m_CommandBundle[k]->SetGraphicsRootConstantBufferView(0, m_ConstantBuffer[k]->GetGPUVirtualAddress() + D3D12_CONSTANT_BUFFER_DATA_PLACEMENT_ALIGNMENT * u);
			m_CommandBundle[k]->DrawIndexedInstanced(m_IndexCount, 1, 0, 0, 0);
		}
