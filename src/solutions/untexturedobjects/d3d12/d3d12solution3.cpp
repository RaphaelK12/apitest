	...
	size_t _objectCount)
{
	...
	m_CopyCommandList->Reset(m_CopyCommandAllocator, 0);
	...
	g_CommandQueue->ExecuteCommandLists(1, (ID3D12CommandList* const*)&m_CopyCommandList);
...
void UntexturedD3D12Solution::Render(const std::vector<Matrix>& _transforms)
{
	...
	if (!m_WorldMatrixBufferData & false)
	{
		// Create a (large) constant buffer
		if (FAILED(g_D3D12Device->CreateCommittedResource(
			&CD3D12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_UPLOAD),
