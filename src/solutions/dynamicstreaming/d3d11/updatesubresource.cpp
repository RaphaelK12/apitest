...
template class DynamicStreamingD3D11UpdateSubresource<GfxApiDirect3D11>;
template class DynamicStreamingD3D11UpdateSubresource<GfxApiDirect3D11On12>;

// --------------------------------------------------------------------------------------------------------------------
// --------------------------------------------------------------------------------------------------------------------
// --------------------------------------------------------------------------------------------------------------------
template<class T>
DynamicStreamingD3D11UpdateSubresource<T>::DynamicStreamingD3D11UpdateSubresource()
: mInputLayout()
, mConstantBuffer()
, mVertexShader()
, mPixelShader()
...
// --------------------------------------------------------------------------------------------------------------------
template<class T>
bool DynamicStreamingD3D11UpdateSubresource<T>::Init(size_t _maxVertexCount)
{
	if (T::m_d3d_device == 0)
		return false;
	...
	hr = CreateDynamicVertexBuffer(mParticleBufferSize, nullptr, D3D11_USAGE_DEFAULT, 0, &mDynamicVertexBuffer, T::m_d3d_device);
    if (FAILED(hr)) {
        return false;
    }
...
// --------------------------------------------------------------------------------------------------------------------
template<class T>
void DynamicStreamingD3D11UpdateSubresource<T>::Render(const std::vector<Vec2>& _vertices)
{
    Constants cb;
    ...
    for (int i = 0; i < kParticleCount; ++i)
    {
        ...
        T::m_d3d_context->UpdateSubresource(mDynamicVertexBuffer, 0, &dstBox, &_vertices[srcOffset], 0, 0);
...
// --------------------------------------------------------------------------------------------------------------------
template<class T>
void DynamicStreamingD3D11UpdateSubresource<T>::Shutdown()
{
